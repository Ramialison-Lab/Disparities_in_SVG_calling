---
title: "Run SPARKX analysis on scanpy processed FF Mouse brain coronal section"
...
orig.ident <- replicate(2513, "ff_mouse_brain_coronal_sections")
...
```{r}
ff_mouse_brain_seurat <- CreateSeuratObject(X,
...
#add active assay
ff_mouse_brain_seurat@active.assay <- c("RNA")
```
...
ff_mouse_brain_seurat@images$slice.1 = new(Class="VisiumV1",assay="RNA",key="image_",coordinates=tissue_coordinates,scale.factors=scale_factors)
ff_mouse_brain_seurat@images$slice.1@assay <- c("RNA")

```

##Load in same data from scanpy X file as scaled data
```{r}
ff_mouse_brain_seurat@assays$RNA@scale.data <- X 
```

#Extract our data with barcodes and information
```{r}
coordinates <- ff_mouse_brain_seurat@images$slice.1@coordinates
...
```{r}
##Need count matrix
counts <- as.matrix(ff_mouse_brain_seurat@assays$RNA@counts)
```
...
#write this to file
write.csv(sig_res,'ff_mouse_brain_coronal_section_sparkx_sig_SVGS_adj_pval.csv')
```