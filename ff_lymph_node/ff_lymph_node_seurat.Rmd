...
orig.ident <- replicate(3782, "ff_lymph_node")
...
```{r}
ff_lymph_node_seurat <- CreateSeuratObject(X,
...
#add active assay
ff_lymph_node_seurat@active.assay <- c("RNA")
```
...
ff_lymph_node_seurat@images$slice.1 = new(Class="VisiumV1",assay="RNA",key="image_",coordinates=tissue_coordinates,scale.factors=scale_factors)
ff_lymph_node_seurat@images$slice.1@assay <- c("RNA")

```

##Load in same data from scanpy X file as scaled data
```{r}
ff_lymph_node_seurat@assays$RNA@scale.data <- X 
```
...
```{r}
ff_lymph_node_seurat@assays$RNA@var.features <- hvg_indices

```

##Try finding spatially variable genes as is
```{r}
ff_lymph_node_seurat <- FindSpatiallyVariableFeatures(ff_lymph_node_seurat, assay = "RNA", features = VariableFeatures(ff_lymph_node_seurat)[1:1000],
    selection.method = "markvariogram")
top.features <- head(SpatiallyVariableFeatures(ff_lymph_node_seurat, selection.method = "markvariogram"), 1000)
saveRDS(top.features,"top_svgs_seurat_ff_lymph_nodes.RDS")
```
...
write.csv(var_subset,'ff_lymph_nodes_seurat_svgs_gene_names.csv')
```